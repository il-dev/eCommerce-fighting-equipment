  ...
  return Object.keys(products).map(key => {
    ...
    return (
      <div
        className={
          loading
            ? "products-item products-item-border opacity-category"
            : "products-item products-item-border"
        }
        key={article_id}
      >
        ...
        <div className="products-item__rate">
          <Link
            href={{
              pathname: `/Product`,
              query: { id: link, descriptionOrReview: false }
            }}
            as={`${link}`}
          >
            <a href={link} className="products-item__feedbacks">
              отзывы
              <span className="products-item__feedbacks-count">{/* 23 */}</span>
            </a>
          </Link>
